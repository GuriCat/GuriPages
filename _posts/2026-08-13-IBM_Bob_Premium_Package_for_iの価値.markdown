...
上の表のとおり、ツール類は「やろうと思えばできる」ものです。しかし「自分で個別に設定できる」と「パッケージとして提供されている」の間には大きな差があります。接続方法を決め、コンパイル定義を書き、権限とガードレールを設計し、動作確認をする——この一連の作業が完了した状態でパッケージ化されている、というのが PP4i の提供形態です。ライブラリー修飾の強制や、承認を要する SQL 種別の既定リストのように、既定値そのものが検討済みの成果物になっています。
